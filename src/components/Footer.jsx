...
const handleSearch = (value) => {
  ...
  let type = value.type;
  const area = value.property_area?.toLowerCase();

  // detect type from link text if API type missing
  if (!type) {
    const link = value.link?.toLowerCase() || value.name?.toLowerCase();

    if (link?.includes("apartment")) type = "apartment";
    else if (link?.includes("villa")) type = "villa";
    else if (link?.includes("plot")) type = "plot";
    else type = "property";
  }
  ...
  navigate(`/${type}-for-sale-in-${area}`);
};
